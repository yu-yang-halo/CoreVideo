...
int CAssistFile::ParseAssistDataForSunplus(string strMOVFile)
{
	...
	/****************sunplus mov file struct ***************************************
	   [ftypSize][ftyp]
	       [mdatSize][mdat]......     --->À˘”–“Ù ”∆µ ˝æ›
		   [moovSize][moov]......     --->Œƒº˛ ˝æ›Õ∑º∞À˜“˝
		   [udatSize][udat]......     --->”√ªß ˝æ›
		   [flagSize][icat6330]       --->sunplusº”µƒŒƒº˛±Í ∂
   ********************************************************************************/
				...
				do {					
					...
					if(track_id == 1) {	//only video tracker
						...
						for(i = 0; i < size; i++) {
							...
							//alladdr[i] = addr + 0x20000; //0x10000: cluster size: 128K   ¡¢ÃÂ…˘: 0x20000
							alladdr[i] = addr + 0x10000; //cluster size: 128K   µ•…˘µ¿: 0x10000
							printf("    idx%d: 0x%08X\n", i, addr);
						}
						gps_rec_cnt = size;
					}
